...
import com.midas.classify.data.ARFFDataSet;
...
public class ARFFStream extends DataStream {
    ...
    while ((line = reader.readLine()) != null) {
      ...
      if (upper.startsWith(ATTRIBUTE_TAG)) {
        ...
        if (upper.endsWith(NOMINAL_DTYPE)) {
          ...
          switch (type) {
            case NUMERIC_DTYPE:
              attributeType.add(DataSet.NUMERIC);
              break;
            case REAL_DTYPE:
              attributeType.add(DataSet.REAL);
              break;
            case INTEGER_DTYPE:
              attributeType.add(DataSet.INTEGER);
              break;
            case STRING_DTYPE:
              attributeType.add(DataSet.STRING);
              break;
          }
        }
      }
  ...
  private ArrayList<String> parse(String src,int maxTokens) throws IOException {
